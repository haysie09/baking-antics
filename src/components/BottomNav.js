...
const NavButton = ({ onClick, label, icon, isActive, className = '' }) => (
    <button
        onClick={onClick}
        className={`flex flex-col items-center justify-center gap-1 w-1/5 pt-2 pb-1 transition-colors duration-200 ${
...
const BottomNav = ({ currentView, navigate, onOpenCreateModal }) => {
    ...
    const icons = {
        ...
        add: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>,
    ...
    return (
        <div className="fixed bottom-0 left-0 z-40 w-full bg-white border-t border-gray-200 font-sans pb-3">
            <div className="flex justify-around items-center h-full max-w-lg mx-auto">
                ...
                <NavButton
                    ...
                    icon={icons.add}
                    isActive={false} // This button is never "active" like the others
                    className="text-[#f0425f]" // Always has the brand color
                />
                <NavButton
                    label="Journal"
